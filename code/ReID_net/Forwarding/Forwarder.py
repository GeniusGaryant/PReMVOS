...
def merge_multi_samples(ys, idx_imgs, target, verbose=True):
  ...
  print("iou@1 samples:", initial_iou, file=log.v5)
  ...
  for k, (y_val, idx_img_val) in enumerate(zip(ys[1:], idx_imgs[1:])):
    ...
    if verbose:
      print("iou@" + str(k + 2), "samples:", curr_iou, file=log.v5)
  if verbose:
    print("iou improvement", curr_iou - initial_iou, file=log.v5)
...
class Forwarder(object, metaclass=ABCMeta):
...
class ImageForwarder(BasicForwarder):
  def __init__(self, engine):
    ...
    self.save_as_png = self.config.bool("save_as_png", False)
  ...
  def _process_forward_result(self, y_argmax, logit, target, tag, extraction_vals, main_folder, save_results):
    ...
    if logit is not None:
      if self.save_as_png:
        ...
        import cv2
        cv2.imwrite(out_fn_logits, (logit[..., 0] * numpy.iinfo(numpy.uint16).max).astype("uint16"))
      else:
  ...
  def _process_forward_minibatch(self, data, network, save_logits, save_results, targets, ys,
    ...
    if self.n_test_samples > 1:
      ...
      ys_argmax_val, logits_val, targets_val, tags_val, n, extraction_vals = self._run_minibatch_single_sample(
        feed_dict, ys, ys_argmax, extractions, targets, network.tags, network.n_imgs)
  ...
  def _run_minibatch_single_sample(self, feed_dict, ys, ys_argmax, extractions, targets, tags, n_imgs):
