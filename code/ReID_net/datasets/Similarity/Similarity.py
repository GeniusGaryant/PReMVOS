...
class SimilarityDataset(Dataset):
    ...
    def __init__(self, config, subset, coord, annotations, n_train_ids, jpg=True):
        ...
        assert self.batching_mode in (
            "single", "pair", "group", "eval"), self.batching_mode
        self.validation_mode = config.str("validation_mode", "embedding")
        assert self.validation_mode in (
            "embedding", "similarity"), self.validation_mode
        ...
        if self.subset != "train":
            context_region_factor_val = config.float(
        ...
        self.bboxes_list = numpy.array(
            [ann["bbox"] for ann in annotations], dtype="float32")
        cat_ids = [ann["category_id"] for ann in annotations]
        self.class_labels_list = numpy.array(cat_ids, dtype="int32")

        train_id_list, train_counts = numpy.unique(
            sorted(cat_ids), return_counts=True)
        # print(set(numpy.arange(0,5952))-set(train_id_list))
        # print(len(train_id_list),len(train_counts))
        # print("started seeming long thing")
        # import time
        # t = time.time()
        self.indices_for_classes = [[idx for idx, id_ in enumerate(
            cat_ids) if id_ == cat_id] for cat_id in train_id_list]
    ...
    def _create_inputs_for_eval(self, batch_size):
        ...
        if USE_DATASET_API:
            ...
            # print("OUTPUT\n\t imgs_norm: {}\n\timgs_raw: {}\n\ttags: {}\n\tclass_labels: {}\n".format(
            #     imgs_norm, imgs_raw, tags, class_labels))
        else:
            ...
            imgs_norm, imgs_raw, tags, class_labels = load_fn(
                fn, tag_, bbox, class_label)
    ...
    def _create_inputs_for_pair(self, batch_size):
        ...
        def _create_example(_=None):
            ...
            sample_same_person = tf.cast(
                tf.equal(sample_same_person, 0), tf.int32)
            ...
            def if_not_same_person():
                ...
                pers_id_2 = tf.cond(pers_id_2 >= pers_id_1,
                                    lambda: pers_id_2 + 1, lambda: pers_id_2)
                pers_2_n_imgs = self.train_counts[pers_id_2 - 1]
                img_id_2 = ((rand[4] - 1) % pers_2_n_imgs) + 1
                return pers_id_2, img_id_2

            pers_id_2, img_id_2 = tf.cond(
                tf.cast(sample_same_person, tf.bool), if_same_person, if_not_same_person)
            ...
            pers_id_1 -= 1
            pers_id_2 -= 1
            img1, img1_class, img1_file_name, original_img1 = self.load_crop(
                img_id_1, pers_id_1)
            img2, img2_class, img2_file_name, original_img2 = self.load_crop(
                img_id_2, pers_id_2)
            tag = img1_file_name + " " + img2_file_name + \
                " " + tf.as_string(sample_same_person)
        ...
        labels = tf.reshape(tf.tile(tf.expand_dims(
            labels, axis=-1), multiples=[1, 2]), [-1])
    ...
    def _create_inputs_for_group(self, batch_size):
        ...
        if batch_size > self.num_train_id:
            sample_from = tf.tile(
                sample_from, [int(ceil(float(batch_size) / self.num_train_id))])
        pers_ids = tf.random_shuffle(sample_from)[0:int(batch_size)]

        def for_each_identity(p_idx):
            pers_id = pers_ids[p_idx]
            img_ids = tf.tile(tf.random_shuffle(
                tf.range(self.train_counts[pers_id])), [4])[:self.group_size]

            def for_each_img(i_idx):
                img_id = img_ids[i_idx]
                img, img_class, img_file_name, original_img = self.load_crop(
                    img_id, pers_id)
                return img, img_class, img_file_name, original_img

            imgs, labels, tags, original_imgs = tf.map_fn(for_each_img, tf.range(
                self.group_size), dtype=(tf.float32, tf.int32, tf.string, tf.float32))
            return imgs, labels, tags, original_imgs

        imgs, labels, tags, original_imgs = tf.map_fn(for_each_identity, tf.range(
            batch_size), dtype=(tf.float32, tf.int32, tf.string, tf.float32))
