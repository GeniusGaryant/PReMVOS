...
class DavisOneshotDataset(OneshotImageDataset):
  def __init__(self, config, subset, use_old_label):
    ...
    self.data_dir = config.str("davis_data_dir", DAVIS_DEFAULT_PATH)
    ...
    self.adaptation_model = config.str("adaptation_model", "")
    self.use_lucid_data = config.int("lucid_interval", -1) != -1
    self.lucid_data_dir = config.str("davis_lucid_data_dir", DAVIS_LUCID_DEFAULT_PATH)
