...
class Davis2017OneshotDataset(OneshotImageDataset):
  def __init__(self, config, subset):
    ...
    self._video = None
    data_dir = config.str("davis_data_dir", DAVIS2017_DEFAULT_PATH)

    self.split = config.str("split", "val")
    ...
    self.lucid_data_dir = config.str("davis_lucid_data_dir", DAVIS2017_LUCID_DEFAULT_PATH)
  ...
  def set_video_idx(self, video_idx):
    ...
    if self._video is None or self.get_video_idx() != video_idx:
      if Davis2017OneshotDataset._cache is not None and Davis2017OneshotDataset._cache[0] == video_idx:
        ...
        #load video
        print("loading sequence", (video_idx + 1), self.video_tag(video_idx), "of davis 2017...", file=log.v2)
        self._video = _load_video(self.imgs_seqs[video_idx], self.ans_seqs[video_idx])
        print("done", file=log.v2)
