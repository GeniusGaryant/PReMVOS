...
class Engine(object):
    def __init__(self, config, seq_path, session=None):
        ...
        self.store_detections_after_each_epoch = config.bool(
            "store_detections_after_each_epoch", False)
        ...
        self.do_oneshot_or_online = self.task in (
            "oneshot_forward", "oneshot", "oneshot_instance", "online")
        ...
        if session is None:
            ...
            if self.tfdbg:
                ...
                self.session = tf_debug.LocalCLIDebugWrapperSession(
                    self.session, thread_name_filter="MainThread$")
                # dump_root="/fastwork/" + username() + "/mywork/data/tmp_tfdbg/")
                self.session.add_tensor_filter(
        ...
        if self.task == "dataset_speedtest":
            self.train_data = load_dataset(
                config, "train", self.session, self.coordinator)
            return
        ...
        if self.use_pre_saved_data or not self.need_val:
            ...
            self.valid_data = load_dataset(
                config, "valid", self.session, self.coordinator, self.seq_path)
			###################################################################
        if self.need_train:
            self.train_data = load_dataset(
                config, "train", self.session, self.coordinator)
        ...
        if not self.use_pre_saved_data and self.valid_data is not None:
            with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
                ...
                print("number of parameters:", "{:,}".format(
                    self.test_network.n_params))
        else:
            self.test_network = None
        self.trainer = Trainer(config, self.train_network,
                               self.test_network, self.global_step, self.session)
        max_saves_to_keep = config.int("max_saves_to_keep", 0)
        self.saver = tf.train.Saver(
            max_to_keep=max_saves_to_keep, pad_step_number=True)
    ...
    def try_load_weights(self):
        ...
        if fn is not None:
            ...
            varlist = [var for var in varlist if var.name.split(
                '/')[0] in good_list]
    ...
    def run_epoch(self, step_fn, data, epoch, train):
        ...
        while n_imgs_processed < n_imgs_per_epoch:
            ...
            if len(res) > 3:
                ...
                if hasattr(data, "set_output_as_old_label"):
                    data.set_output_as_old_label(
                        tags, ys_armax_val, epoch, targets)
            else:
            ...
            measures_avg = Measures.calc_measures_avg(
                measures, n_imgs, ignore_classes, for_final_result=False)
            end = time.time()
            elapsed = end - start

            # TODO: Print proper averages for the measures
            print(n_imgs_processed, '/', n_imgs_per_epoch,
                  loss_avg, measures_avg, "elapsed", elapsed)
            ...
            if self.cmc_validation:
                ...
                for valid_round in range(3):
                    valid_loss, valid_measures = self.run_epoch(
                        self.trainer.validation_step, self.valid_data, epoch, train=False)
    ...
    def eval(self):
        start = time.time()
        if self.cmc_validation:
            valid_loss, measures = do_cmc_validation(
                self, self.test_network, self.valid_data)
        else:
            valid_loss, measures = self.run_epoch(
                self.trainer.validation_step, self.valid_data, 0, train=False)
        end = time.time()
        elapsed = end - start
        valid_error_string = Measures.get_error_string(measures, "valid")
        print("eval finished. elapsed:", elapsed,
    ...
    def run(self):
        if self.task == "train" or self.task == "train_no_val":
            ...
            forward(self, network, data, self.dataset,
        ...
        ##########################################################
            ...
            if self.task == Constants.FORWARD_INTERACTIVE:
                ...
                save_results = self.config.bool("save_results", True)
            interactive_forward(self, network, data,
                                save_results, save_logits, self.task)

        elif self.task == Constants.ONESHOT_INTERACTIVE:
            oneshot_forwarder = OneshotForwarder.InteractiveOneshotForwarder(
                self)
            ...
            save_logits = self.config.bool("save_logits", False)
            save_results = self.config.bool("save_results", True)
            iterative_forwarder = IterativeImageForwarder.IterativeImageForwarder(
                self)
            iterative_forwarder.forward(network=self.test_network, data=self.valid_data,
                                        save_results=save_results, save_logits=save_logits)
        elif self.task == "dataset_speedtest":
            forwarder = DatasetSpeedtestForwarder.DatasetSpeedtestForwarder(
                self)
            ...
            if self.task == "oneshot":
                oneshot_forward(self, save_results=save_results,
                                save_logits=save_logits)
            elif self.task == "online":
                online_forward(self, save_results=save_results,
                               save_logits=save_logits)
            else:
                offline_forward(self, save_results=save_results,
                                save_logits=save_logits)
    ...
    def _maybe_store_detections(self, epoch, train, measures):
        ...
        if self._detection_file is None:
            ...
            det_file_path = "forwarded/" + self.model + \
                "/det_" + str(epoch + 1) + ".json"
