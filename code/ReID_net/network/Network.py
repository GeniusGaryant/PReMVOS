...
class Network(object):
    ...
    def build_tower(self, network_def, x_image, y_ref, tags, void_label, n_classes, tower_setup):
        ...
        with tf.device(gpu_str), tf.name_scope("tower_gpu_" + str(tower_setup.gpu)):
            ...
            for name, layer_def in list(network_def.items()):
                ...
                use_image_summaries = self.config.bool(
                    "use_image_summaries", False)
                ...
                if "alternative_labels" in layer_def:
                    ...
                    layer_def["targets"] = sum(
                        [layers[x].out_labels for x in layer_def["alternative_labels"]])
    ...
    def build_network(self, config, x_image, y_ref, tags, void_label, n_classes, is_training, freeze_batchnorm,
        ...
        if not is_training:
            gpus = gpus[:1]
        if self.use_partialflow:
            # partialflow does not work with multigpu
            assert len(gpus) == 1, len(gpus)
        network_def = config.dict("network")
        batch_size_tower = self.batch_size / len(gpus)
        assert batch_size_tower * \
            len(gpus) == self.batch_size, (batch_size_tower,
                                           len(gpus), self.batch_size)
        ...
        for idx, gpu in enumerate(gpus):
            ...
            if len(gpus) == 1:
                ...
                stride = batch_size_tower * \
                    (1 if self.chunk_size == -1 else self.chunk_size)
                x_image_tower = x_image[idx * stride:(idx + 1) * stride]
                tags_tower = tags[idx * stride:(idx + 1) * stride]
                if original_sizes is not None:
                    original_sizes = original_sizes[idx *
                                                    stride:(idx + 1) * stride]
                if resized_sizes is not None:
                    resized_sizes = resized_sizes[idx *
                                                  stride:(idx + 1) * stride]
                if isinstance(y_ref, tuple):
                    y_ref_tower = tuple(
                        v[idx * stride: (idx + 1) * stride] for v in y_ref)
                else:
            ...
            if isinstance(y_softmax_total[0], tuple):
                ...
                for n in range(len(y_softmax_total[0])):
                    if y_softmax_total[0][n] is None:
                        ...
                        v = tf.concat(axis=0, values=[y[n]
                                                      for y in y_softmax_total])
                        y_softmax_out.append(v)
                y_softmax_total = tuple(y_softmax_out)
            else:
                y_softmax_total = tf.concat(
                    axis=0, values=y_softmax_total, name='y_softmax_total')
        if self.current_graph_section is not None:
    ...
    def __init__(self, config, dataset, global_step, training, use_partialflow=False, freeze_batchnorm=False,
        ...
        with tf.name_scope(name):
            ...
            if use_partialflow:
                import partialflow
                self.graph_section_manager = partialflow.GraphSectionManager(
                    verbose=False)
                self.graph_sections = []
            else:
            ...
            if training:
                ...
                do_multi_sample_testing = config.int(
                    "n_test_samples", -1) != -1
            ...
            self.inputs_tensors_dict = dataset.create_input_tensors_dict(
                self.batch_size)
            ...
            self.img_ids = self.inputs_tensors_dict.get(
                Constants.IMG_IDS, None)
            ...
            if self.raw_labels is not None and not isinstance(self.y_softmax, tuple):
                self.ys_resized = tf.image.resize_images(
                    self.y_softmax, tf.shape(self.raw_labels)[1:3])
